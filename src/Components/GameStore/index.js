...
class GameStore extends Component {
  ...
  getShuffledImage = () => {
    const {thumbnailImagesList} = this.props
    const shuffledImageNumber = Math.ceil(
      Math.random() * (thumbnailImagesList.length - 1),
    )
    return thumbnailImagesList[shuffledImageNumber].imageUrl
  }

  finishGameAndSetScore = () => {
    this.setState({isGameInProgress: false})
  }

  setActiveImageId = imgUrl => {
    const shuffledImage = this.getShuffledImage()
    const {imageUrl} = this.state
    if (imageUrl === imgUrl) {
      this.setState(prevState => ({
        points: prevState.points + 1,
        imageUrl: shuffledImage,
      }))
    } else {
      this.finishGameAndSetScore()
      clearInterval(this.timerId)
    }
  }
  ...
  render() {
    const {points, imageUrl, time, activeTabId, isGameInProgress} = this.state
    ...
    return (
      <div className="app-container">
          ...
          <ul className="score-container">
            ...
            <li className="score-card">
              <img
                ...
                className="timer"
              />
              <p className="seconds">{time} Sec</p>
            </li>
          </ul>
        </div>
        <div className="game-container">
          {isGameInProgress && (
            <>
              <img src={imageUrl} alt="match" className="current-image" />
